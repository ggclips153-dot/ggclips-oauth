...
|---|---|---|
| 1 | The brief says a deleted agent's **name is retired forever**, but the New Agent form says the name "can repeat across time". Which wins? | Strict: a deleted agent's name can never be reused (case-insensitive). Living agents may share a name. |
...
| 8 | Families for Personal Finance City and GGClutchPlays? | Both `revenue` in `config/seed.example.json`. |
| 9 | Mayor names for the three seed cities? | Required before seeding (`config/seed.json`). |
...
| 11 | Structure for PROPOSAL-2026-09-25-B-REV2 (four-tier spec) and the SOULs | Waiting on Marc's upload; district/department fields will be reconciled when received. |
