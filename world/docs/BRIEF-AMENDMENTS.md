...
| A16 | 2026-09-25 | **Replacing a dean**: an outstanding professor of that college becomes dean (Marc's routed intent; the Mayor executes). The outgoing dean returns to teaching as a professor. The new dean starts a fresh scorecard and keeps its teaching record. A college without a dean can take its first one this way too. **The Mayor's reviews of a dean can have consequences**; which ones will be decided after testing. | `intent.replace_dean`, `dean.replaced` |
| A17 | 2026-09-25 | **Claude and Gemini hold one city each.** The map gives Revenue the most room: a full-width home on the 2D map and half the island in 3D; Essentials a quarter; Claude and Gemini an eighth each. A second Claude or Gemini city is rejected. | `MAX_CITIES_PER_FAMILY` |
| A18 | 2026-09-25 | **Economy settings**: the currency is **dollars** (kept in cents); earning and reward **amounts are set per grant** by the Mayor + Marc for now; the clean-attribution **period is one week** (weeks start Monday). | `CURRENCY`, `work.deliverable`, `work.qc_rework`, `currency.earned`, `currency.spent` |
