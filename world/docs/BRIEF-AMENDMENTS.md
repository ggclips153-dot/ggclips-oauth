...
| A12 | 2026-09-25 | **Professors follow the same strike consequences as everybody else**: Security's task strikes and jail ladder, and 3 strikes = held awaiting deletion, deleted only on Marc's routed intent. A jailed professor can't examine or step in. Professors transfer **within their city only** (change of specialty). | professors are agent records with `role: 'professor'`; `intent.specialize_professor` |
| A13 | 2026-09-25 | **Professors have their own strike rules.** They don't take KPI strikes; they take *teaching strikes* under the college's rules (the full school system is being designed by Marc). Each professor has a **teaching record**: exams given, exams passed, and students graduated under it. | `professor.strike`, `teaching` on the professor record |
