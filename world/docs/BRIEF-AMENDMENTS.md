...
| A10 | 2026-09-25 | **Professors**: custom-created specialists for one department. They teach, give exams and judge whether a student is fit to graduate; graduation needs a passed exam from a professor of that department. They can step in to fill a role for a set time. They share the never-reused agent ID space but are not graduated agents (not capped, not counted toward dept-lead). | `professor.enrolled`, `exam.graded`, `professor.stepped_in` |
